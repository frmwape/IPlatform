...
namespace GameOfLife.Rules
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger<ZombiePopulationRule>();
        ...
        private void ApplyRule(World world, WorldRow row)
        {
            ...
            foreach (var cell in row.Cells)
            {
                ...
                if ((livingCells < 2 || livingCells > 3) && cell.State.GetType() == baseType)
                {
                    log.InfoFormat("Zombify {0}, {1} because less than 2 neighbours or more than 3 neighbours, got {2} neighbours and is in {3}",
                        cell.CellNumber,
                        cell.RowNumber,
                        livingCells,
                ...
                else if ((livingCells < 2 || livingCells > 3) && cell.State.GetType() == typeof(ZombieState))
                {
                    log.InfoFormat("Killing {0}, {1} because less than 2 neighbours or more than 3 neighbours, got {2} neighbours and is in {3}",
                        cell.CellNumber,
                        cell.RowNumber,
                        livingCells,
                        cell.State.GetType().ToString().Replace("GameOfLife.Worlds.Cells.", ""));
